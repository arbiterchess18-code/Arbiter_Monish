...
def calculate_standings(db: Session, tournament_id: int):
    ...
    player_stats = {uid: {
        ...
        # opp_id -> result weight (1, 0.5, 0)
        "direct_encounter_opponents": {},
        "rating": 0
        ...
        if w_id in player_stats:
            if res == "1-0":
                ...
                if b_id in player_stats:
                    player_stats[w_id]["direct_encounter_opponents"][b_id] = 1.0
            elif res == "0-1":
                if b_id in player_stats:
                ...
                if b_id in player_stats:
                    player_stats[w_id]["direct_encounter_opponents"][b_id] = 0.0
                    player_stats[b_id]["direct_encounter_opponents"][w_id] = 0.0
            elif res == "Bye":
                player_stats[w_id]["points"] += 1.0
                player_stats[w_id]["wins"] += 1
    ...
    total_rounds = db.query(models.Round).filter(
        models.Round.tournament_id == tournament_id).count()
    for uid in player_ids:
        prog = 0.0
        curr = 0.0
    ...
    for uid in player_ids:
        ...
        opp_ratings = [player_stats[oid]["rating"]
                       for oid in stats["opponents"]]

        bh_total = sum(opp_points)
        bh_cut1 = bh_total - min(opp_points) if opp_points else 0.0
        bh_cut2 = bh_total - sum(sorted(opp_points)
                                 [:2]) if len(opp_points) >= 2 else 0.0
        ...
        base_name = f"{player.first_name or ''} {player.last_name or ''}".strip(
        ) or player.username
    ...
    standings_data.sort(key=lambda x: get_standings_sort_key(
        x, tb_config), reverse=True)
        ...
        for i in range(len(standings_data) - 1):
            ...
            if p1["points"] == p2["points"]:
                ...
                if res == 0.0:  # p2 won against p1
                    standings_data[i], standings_data[i +
                                                      1] = standings_data[i+1], standings_data[i]
