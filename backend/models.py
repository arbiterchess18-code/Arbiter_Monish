...
class Tournament(Base):
    ...
    pairing_system = Column(String(50), default="Swiss") # Swiss, Round Robin, Knockout
    ...
    status = Column(String(30), default="upcoming") # upcoming, active, completed
    created_at = Column(TIMESTAMP, server_default=func.now())
    updated_at = Column(TIMESTAMP, server_default=func.now(), onupdate=func.now())

    # Relationships
    creator = relationship("User", back_populates="created_tournaments")
    rounds_list = relationship("Round", back_populates="tournament", cascade="all, delete-orphan")
...
class Match(Base):
    ...
    tournament_id = Column(Integer, ForeignKey("tournaments.tournament_id", ondelete="CASCADE"))
    round_id = Column(Integer, ForeignKey("rounds.round_id", ondelete="CASCADE"))
    ...
    white_player = relationship("User", foreign_keys=[white_player_id], back_populates="white_matches")
    black_player = relationship("User", foreign_keys=[black_player_id], back_populates="black_matches")
