...
import {
    PlusCircle, ClipboardList, Users, ShieldAlert,
    Settings, BarChart3, Clock, CheckCircle2
} from "lucide-react";
...
const ArbiterDashboard = () => {
    ...
    const managedTournaments = mockTournaments.filter(t => t.status === "active" || t.status === "upcoming").slice(0, 2);
    ...
    return (
        <div className="space-y-8 pb-10">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                ...
                />
                <Button onClick={() => navigate("/orbiter/create")} className="gap-2 rounded-full px-6 h-11">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 space-y-4">
                    <div className="flex items-center justify-between">
                        <h2 className="text-xl font-display font-bold">My Managed Tournaments</h2>
                        <Button variant="link" onClick={() => navigate("/orbiter/manage")}>View All</Button>
                    </div>
            ...
            <section className="stat-card">
                ...
                <div className="space-y-4">
                    {[
                        { time: "2 mins ago", event: "Anti-cheat check passed for Round 5 (Table 1)", icon: ShieldAlert, color: "text-green-500" },
                        { time: "15 mins ago", event: "Manual result override needed for Tournament #12", icon: Settings, color: "text-amber-500" },
                        { time: "1 hour ago", event: "New player request for 'Spring Invitational'", icon: Users, color: "text-blue-500" },
                    ].map((item, idx) => (
                        <motion.div
                            key={idx}
                        ...
                        >
                            ...
                            <div>
                                ...
                                <p className="text-[10px] text-muted-foreground uppercase tracking-wider mt-0.5">{item.time}</p>
                            </div>
                        </motion.div>
                    ))}
                </div>
