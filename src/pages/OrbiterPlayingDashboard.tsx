...
import { mockTournaments, mockOrbiterPlayingStats, mockRatingHistory, Tournament } from "@/lib/mock-data";
...
export default function OrbiterPlayingDashboard() {
  ...
  return (
    <div className="space-y-8">
      <PageHeader title="Playing Dashboard" description="Your personal competitive performance as a player" />

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Current Rating" value={stats.currentRating} icon={TrendingUp} delay={0} />
        <StatCard title="Matches Played" value={stats.totalMatches} icon={Swords} delay={0.1} />
        <StatCard title="Win Rate" value={`${Math.round((stats.wins / stats.totalMatches) * 100)}%`} icon={BarChart3} delay={0.15} />
        <StatCard title="Tournaments Played" value={stats.totalTournaments} icon={Trophy} delay={0.2} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="stat-card">
          ...
          <div className="space-y-4">
            {[
              { label: "Wins", value: stats.wins, total: stats.totalMatches, color: "bg-success" },
              { label: "Draws", value: stats.draws, total: stats.totalMatches, color: "bg-warning" },
              { label: "Losses", value: stats.losses, total: stats.totalMatches, color: "bg-destructive" },
            ].map(stat => (
              <div key={stat.label}>
      ...
      <div className="space-y-4">
        <h2 className="font-display font-semibold text-lg">Tournaments to Join as Player</h2>
